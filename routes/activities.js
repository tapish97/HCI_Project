...
router.get("/:tripid", async (req, res) => {
  ...
  for (let x in theActivities) {
    ...
    if (theActivities[x].type == "flight") {
      ...
      theActivities[x].info.depart_date_time =
        theActivities[x].info.departure_date +
        " " +
        theActivities[x].info.departure_time +
        " " +
        theActivities[x].info.departure_time_zone;
      theActivities[x].info.arrives_date_time =
        theActivities[x].info.arrival_date +
        " " +
        theActivities[x].info.arrival_time +
        " " +
        theActivities[x].info.arrival_time_zone;
    ...
    if (theActivities[x].type == "tour_event") {
      ...
      let theDate = theActivities[x].info.start_date_time
        .split(" ")[0]
        .split("-");
      selectionDates.push(theDate[1] + "-" + theDate[2]);
    }
    if (theActivities[x].type == "bus_boat") {
      ...
      let theDate = theActivities[x].info.depart_date_time
        .split(" ")[0]
        .split("-");
      selectionDates.push(theDate[1] + "-" + theDate[2]);
    }
  ...
  if (start_date.getMonth() == end_date.getMonth())
    theTrip.end_day = end_date.getDate() + 1;
  else
    theTrip.end_day =
      monthNames[end_date.getMonth()] + " " + end_date.getDate() + 1;
  ...
  for (let x in theActivities) {
    ...
    if (theActivities[x].type == "flight") {
      ...
      theActivities[x].info.arrives_date_time =
        ...
        theActivities[x].info.arrival_time_zone;
      let theDate = theActivities[x].info.departure_date.split("-");
      if (req.params.date.localeCompare(theDate[1] + "-" + theDate[2]) != 0)
        theActivities[x] = null;
      selectionDates.push(theDate[1] + "-" + theDate[2]);
...
router.post("/:tripid/:type", async (req, res) => {
  try {
    ...
    if (type == "flight") {
      ...
      created = await activityData.create(
      ...
      theInfo.depart_date_time =
        req.body.departure_date + " " + req.body.departure_time;
      theInfo.arrives_date_time =
        req.body.arrival_date + " " + req.body.arrival_time;
...
router.post("/edit/:tripid/:id/:type", async (req, res) => {
  ...
  try {
    ...
    if (type == "flight") {
      ...
      updated = await activityData.update(
        req.session.user._id,
        tripid,
        id,
        theInfo
      ...
      updated = await activityData.create(
        req.session.user._id,
        tripid,
        "bus_boat",
        theInfo
